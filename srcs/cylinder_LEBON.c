...
/*   Updated: 2024/02/06 01:43:38 by octonaute        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

/* Function that creates an orthonormal basis given a single vector :
the cylinder's direction. To do so, it creates two arbitrary vectors not
parallel to the cylinder's direction */
...
model of coordonates vs real world coordonates) : the cylinder will be at
the origin (0,0,0) in local space to facilitate further calculations */
t_ray transform_ray_to_local(t_ray world_ray, t_objs *cylinder) {
	...
	// printf("Before transformation: world_origin = (%f, %f, %f), dir = (%f, %f, %f)\n",
    //    world_ray.origin.x, world_ray.origin.y, world_ray.origin.z,
    //    world_ray.dir.x, world_ray.dir.y, world_ray.dir.z);

    create_basis(cylinder->dir, &up, &right);
    local_origin = vec_substract(world_ray.origin, cylinder->pos);

	// printf("Before transformation: local_origin = (%f, %f, %f), dir = (%f, %f, %f)\n",
    //    local_ray.origin.x, local_ray.origin.y, local_ray.origin.z,
    //    local_ray.dir.x, local_ray.dir.y, local_ray.dir.z);
	...
	// local_ray.origin = vec_normalize(local_ray.origin); //REGLE X EN DIR 0,0,1 MAIS DÉRRÈGLE LES OMBRES

	// printf("After transformation: origin = (%f, %f, %f), dir = (%f, %f, %f)\n",
    //    local_ray.origin.x, local_ray.origin.y, local_ray.origin.z,
    //    local_ray.dir.x, local_ray.dir.y, local_ray.dir.z);
...
void intersection_point_cylinder(t_inter *inter, t_objs *cylinder)
{
	...
	// printf("Rendering cylinder at position (%f, %f, %f)\n",
