...
// 			// initial_distance = vec_pythagore(vecSubstract(data->closest_intersection_point, light->position));
// 			initial_distance = vec_pythagore(vecSubstract(light->position, data->closest_intersection_point));
...
// 						new_distance = vec_pythagore(vecSubstract(new_intersection, light->position));
...
bool	intersection(t_data *data)
{
	bool	intersection;
	t_objs	*object;
	// t_objs	*light_sphere; //TODO temporaire
